...
export class ClientsController {
  ...
  // SYNC ENDPOINTS - Must come before :id routes
  ...
  @Delete('batch/delete')
  @ApiOperation({ summary: 'Delete multiple clients at once (soft delete)' })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        ids: {
          type: 'array',
          items: { type: 'string' },
          description: 'Array of client UUIDs to delete',
        },
      },
      required: ['ids'],
    },
  ...
  @ApiResponse({
    ...
    schema: {
      type: 'object',
      properties: {
        count: { type: 'number', description: 'Number of clients deleted' },
        deletedIds: { type: 'array', items: { type: 'string' } },
      },
    },
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  removeMany(@CurrentUser() user: any, @Body('ids') ids: string[]) {
    return this.clientsService.removeMany(user.id, ids);
  }
}
