...
@ApiTags('PDF Jobs (Local Mode)')
@ApiBearerAuth('JWT-auth')
@Controller()
@UseGuards(JwtAuthGuard)
export class PdfJobsLocalController {
  constructor(private pdfJobsService: PdfJobsLocalService) {}

  @Post('quotes/:id/generate-pdf-async')
  @HttpCode(HttpStatus.OK) // 200 instead of 202 since it's synchronous
  @ApiOperation({ summary: 'Gerar PDF do orçamento (modo local/síncrono)' })
  ...
  @ApiOperation({ summary: 'Gerar PDF da OS (modo local/síncrono)' })
  @ApiResponse({ status: 200, description: 'PDF gerado com sucesso' })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Gerar PDF da fatura (modo local/síncrono)' })
  @ApiResponse({ status: 200, description: 'PDF gerado com sucesso' })
