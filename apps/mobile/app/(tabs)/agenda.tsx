...
function formatDateKey(date: Date): string {
  // Usa formatação local para evitar problemas de UTC
...
export default function AgendaScreen() {
  ...
  const loadDayWorkOrders = useCallback(async (date: Date) => {
    ...
    try {
      ...
      // If there's a datatype mismatch error, try to reset the database
  ...
  const loadWeekWorkOrders = useCallback(async (week: Date[]) => {
    try {
      ...
      // Group by date - use scheduledDate or scheduledStartTime
      const grouped: Record<string, WorkOrder[]> = {};
      orders.forEach((wo) => {
        // Get date from scheduledDate or scheduledStartTime usando utilitário para evitar problemas de UTC
